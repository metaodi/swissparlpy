...
try:
    ...
    warnings.warn(
        "matplotlib is not installed. Install it with 'pip install matplotlib' to use visualization features.",
        ImportWarning
    )
...
def plot_voting(votes, seats=None, highlight=None, result=False, result_size=12,
                point_shape='o', point_size=50, theme='scoreboard', ax=None):
    """
    Plot voting results of the Swiss National Council according to the seating order.
    
    This function visualizes voting results in the Swiss National Council chamber,
    showing how each councillor voted (yes/no/abstention) using different themes.
    ...
    highlight : dict, optional
        Dictionary with variable name and values to highlight selected councillors.
        Example: {'ParlGroupNumber': [2]} to highlight a parliamentary group.
    result : bool, default False
        If True, annotate the plot with voting results (yes/no/abstention counts).
    ...
    theme : str, default 'scoreboard'
        ...
        - 'scoreboard': Imitates the council hall scoreboard with neon colors on black
    ...
    data = seating_plan.merge(seats[['SeatNumber', 'PersonNumber']], on='SeatNumber', how='left')
    ...
    if highlight is not None:
        if not isinstance(highlight, dict) or len(highlight) != 1:
            raise ValueError("highlight must be a dictionary with exactly one key-value pair")
...
def _get_theme_config(theme):
    ...
    # Define decision colors for each theme
    decision_colors = {
        1: None,  # Yes - will be set per theme
        2: None,  # No - will be set per theme  
        3: None,  # Abstention - will be set per theme
        4: None,  # Did not vote
        5: None,  # Excused
        6: None,  # President
        7: None,  # Other
    ...
    configs = {
        'scoreboard': {
            'colors': {
                ...
                4: 'black', 5: 'black', 6: 'black', 7: 'black', 8: 'black'
            },
            'seat_fill': 'black',
            'seat_edge': 'white',
            'bg_color': 'black',
        ...
        'sym1': {
            'colors': {
                ...
                2: '#D55E00',  # Orange/red
                3: 'grey',     # Grey
                4: 'white', 5: 'white', 6: 'white', 7: 'white', 8: 'white'
            },
            'seat_fill': 'white',
            'seat_edge': 'black',
            'bg_color': 'white',
        ...
        'poly1': {
            'colors': {
                ...
                2: '#D55E00',  # Orange/red
                3: 'grey',     # Grey
                4: '#f0f0f0', 5: '#f0f0f0', 6: '#f0f0f0', 7: '#f0f0f0', 8: '#f0f0f0'
            },
    ...
    if theme not in configs:
        raise ValueError(f"Unknown theme '{theme}'. Choose from: {list(configs.keys())}")
...
def _add_results(data, ax, theme_config, result_size, has_highlight):
    ...
    vote_counts = unique_persons[unique_persons['Decision'].isin([1, 2, 3])].groupby(
        ['Decision', 'DecisionText']
    ).size().reset_index(name='count')
    
    # If highlighting, also count highlighted votes
    if has_highlight:
        highlighted_counts = unique_persons[
            (unique_persons['Decision'].isin([1, 2, 3])) & 
            (unique_persons['highlight'] == 0)
        ].groupby(['Decision', 'DecisionText']).size().reset_index(name='highlight_count')
        vote_counts = vote_counts.merge(highlighted_counts, on=['Decision', 'DecisionText'], how='left')
        vote_counts['highlight_count'] = vote_counts['highlight_count'].fillna(0).astype(int)
